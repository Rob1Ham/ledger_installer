    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ChannelMismatch { expected, got } => {
                write!(
                    f,
                    "Channel mismatch: expected {:#06x}, got {:#06x}",
                    expected, got
                )
            }
            Self::InvalidTag { expected, got } => {
                write!(
                    f,
                    "Invalid tag: expected {:#04x}, got {:#04x}",
                    expected, got
                )
            }
            ...
            Self::DeviceError { sw, message } => {
                write!(f, "Device error (SW={:#06x}): {}", sw, message)
            }
...
pub fn parse_response(packets: &[&[u8]], channel_id: u16) -> Result<(Vec<u8>, u16), ApduError> {
    ...
    for (i, packet) in packets.iter().enumerate() {
        ...
        // Verify channel ID
        let pkt_channel = ((packet[0] as u16) << 8) | (packet[1] as u16);
        ...
        let pkt_sequence = ((packet[3] as u16) << 8) | (packet[4] as u16);
    ...
    let sw = ((data[sw_offset] as u16) << 8) | (data[sw_offset + 1] as u16);
...
pub fn interpret_status_word(sw: u16) -> Result<(), ApduError> {
    match sw {
        ...
        _ => Err(ApduError::DeviceError {
            sw,
            message: format!("Unknown status word: {:#06x}", sw),
        }),
    }
}
